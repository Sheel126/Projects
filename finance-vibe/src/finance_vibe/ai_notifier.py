...
def get_latest_attachment_file(mode: str, root_dir: Path) -> Path | None:
    """Finds the most recent AI Studio markdown report or fallback trade plan CSV."""
    log_dir = root_dir / "data" / "logs" / mode
    if not log_dir.exists():
        return None

    # Priority 1: AI Studio Reports (*.md)
    reports = sorted(
        list(log_dir.glob("ai_studio_report_*.md")), key=os.path.getmtime, reverse=True
    )
    if reports:
        return reports[0]

    # Priority 2: Clean or Raw Trade Plans (*.csv)
    plans = sorted(
        list(log_dir.glob("trade_plan_*.csv")), key=os.path.getmtime, reverse=True
    )
    if plans:
        return plans[0]
...
def analyze_with_gemini(combined_data: str) -> str:
    ...
    3. Senior Market Analyst Validation & Qualitative Insights (Top 5 ML Ranked Tickers ONLY):
...
def send_html_email(
    subject: str, html_content: str, attachment_path: Path | None = None
):
    """Dispatches HTML email via Gmail SMTP with optional file attachment."""
    ...
    if attachment_path and attachment_path.exists():
        ...
        print(f"📎 Attached file to email: {attachment_path.name}")
...
def main():
    ...
    attachment_file = get_latest_attachment_file(args.mode, root_dir)

    print("📧 Dispatching full master HTML report...")
    subject = f"📊 Finance-Vibe [{args.mode.upper()}] Master Pipeline Briefing"
    send_html_email(subject, html_insights, attachment_path=attachment_file)
