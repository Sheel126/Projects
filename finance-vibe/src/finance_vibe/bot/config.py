...
STRATEGY_NOTES = os.getenv(
    "BOT_STRATEGY_NOTES",
    "Day-3 daily-active: 5 positions max, dip-buys on volatile names, "
    "take profit ~1.2%+, cut losers ~1.8%, flat by 3:55 PM. "
    "No new buys when QQQ < -0.4% from open or after 3:30 PM. Software stops only.",
)
...
ACTIVE_MAX_BUYS_PER_CYCLE = int(os.getenv("BOT_ACTIVE_MAX_BUYS_PER_CYCLE", "3"))
ACTIVE_MIN_BUY_SCORE = float(os.getenv("BOT_ACTIVE_MIN_BUY_SCORE", "28"))
ACTIVE_MIN_RSI = float(os.getenv("BOT_ACTIVE_MIN_RSI", "25"))
ACTIVE_MAX_RSI = float(os.getenv("BOT_ACTIVE_MAX_RSI", "65"))
ACTIVE_SELL_RSI = float(os.getenv("BOT_ACTIVE_SELL_RSI", "70"))
ACTIVE_SELL_FROM_OPEN_PCT = float(os.getenv("BOT_ACTIVE_SELL_FROM_OPEN_PCT", "2.0"))
